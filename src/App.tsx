...
const App:React.FC = () => {
    ...
    return (
        <div>
        <Editor onChange={(inp:string)=>setInput(inp)} onSubmitHandle={() => submitButtonRef.current?.click()}/>
         <textarea onChange={(e:any)=>setInput(e.target.value)} value={input} onKeyUp={e=>{
                if(e.key === 'Enter' && e.ctrlKey){
                    onSubmitListener();
                }
         }}></textarea>
         <button ref={submitButtonRef} onClick={ onSubmitListener }>RUN</button>
